...
def preprocessing_context():
    ...
    for f in source_files:
        ...
        outpath = os.path.join('..', 'saved_data/source-tweets-context-top25/{}'.format(event))
        os.makedirs(outpath, exist_ok=True)
        np.save(os.path.join(outpath, 'ids1'), ids)
        np.save(os.path.join(outpath, 'train_array1'), final_features)
        np.save(os.path.join(outpath, 'rnr_labels1'), labels)
        print("")
