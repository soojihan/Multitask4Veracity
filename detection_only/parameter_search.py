...
def parameter_search(ntrials, objective_function, fname):
    ...
    directory = "/mnt/fastdata/acp16sh/Multitask4Veracity-master/output-context-gridsearch"
    if not os.path.exists(directory):
        os.mkdir(directory)

    f = open('/mnt/fastdata/acp16sh/Multitask4Veracity-master/output-context-gridsearch/trials_' + fname + '.txt', "wb")
    pickle.dump(trials, f)
    f.close()

    filename = '/mnt/fastdata/acp16sh/Multitask4Veracity-master/output-context-gridsearch/bestparams_' + fname + '.txt'
    f = open(filename, "wb")
